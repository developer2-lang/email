...
//   1. filters the Contacts table by the segment's category (never the whole
//      table for a specific segment),
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const contacts = await resolveContactsForCampaign(saved.id, saved.audience_segment);
