...
// ─── Automatic backfill (existing openers) ──────────────────────────────────
// Mirrors backend/followupService.js processAutomaticFollowup: when an automatic
// follow-up is created, every contact who has ALREADY opened the original
// campaign receives the follow-up immediately — it must not wait for a future
// open event or a scheduler tick. Duplicate protection (sent/already_sent) keeps
// each opener from being emailed twice. Used by the normal Create Follow-up flow
// and the CSV/Excel Import flow so both initialise identically.
async function handleProcessAutomatic(payload: any): Promise<{ sent: number; skipped: number; failed: number }> {
  const originalCampaignId = payload.campaign_id ? String(payload.campaign_id) : '';
  const followupCampaignId = payload.followup_campaign_id ? String(payload.followup_campaign_id) : '';
  if (!originalCampaignId || !followupCampaignId) {
    throw new Error('campaign_id and followup_campaign_id are required');
  }
  ...
  const { data: openedLogs, error: openedError } = await supabase
    ...
    .eq('campaign_id', originalCampaignId)
    .eq('opened', true);
  if (openedError) throw new Error(`Failed to fetch openers: ${openedError.message}`);
  ...
  for (const openedLog of openedLogs || []) {
    ...
    const email = (openedLog.email as string) || contact.email || '';

    try {
      await sendFollowupEmail(followupCampaignId, contactId, email, mimeAttachments);
      const sentAt = new Date().toISOString();
      let logRow = existing;
      if (!logRow) {
        logRow = await insertFollowupLog({
          campaignId: originalCampaignId,
          contactId,
          email: email || (openedLog.email as string) || '',
          followupCampaignId,
          openedAt: (openedLog.opened_at as string) || null,
        });
      }
      if (logRow && logRow.id) {
        await updateFollowupLog(logRow.id, { status: 'sent', sent_at: sentAt });
      } else {
        const refetched = await getFollowupLog(originalCampaignId, contactId, followupCampaignId);
        if (refetched && refetched.id) {
          await updateFollowupLog(refetched.id, { status: 'sent', sent_at: sentAt });
        }
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      logErr(`process_automatic send FAILED for ${email}: ${message}`);
      failed++;
    }
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'process_automatic') {
      const data = await handleProcessAutomatic(payload);
      return respond(200, { success: true, data });
    }
