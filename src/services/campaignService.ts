...
/**
 * Read an .html/.htm template file into memory WITHOUT persisting it. Used by
 * the Template Editor's "Import File" feature: it parses and validates the file
 * the same way `uploadEmailTemplate` does, but performs NO database or Storage
 * write — the raw HTML is handed back so it can be loaded straight into the
 * existing Visual Editor. Reuses the same extension + content validation used
 * by the upload path so behaviour stays consistent.
 */
export async function readHtmlTemplateFile(
  file: File
  ...
  if (!HTML_TEMPLATE_EXT_RE.test(fileName)) {
    throw new Error(
      'Invalid file type. Please upload an HTML template (.html or .htm).'
    )
  }
  if (file.size <= 0) {
    throw new Error(`'${fileName}' is empty and cannot be imported.`)
  }

  const html = await file.text()
  if (!isValidEmailHtml(html)) {
    throw new Error(
      'Unable to read this HTML template. Please check the file and try again.'
    )
  }

  return { html, name: templateNameFromFilename(fileName) }
}
...
/**
 * Persist a recurring schedule row for an already-created campaign, reusing the
 * exact same schedule-construction logic (`buildScheduleInput` + `buildScheduleRow`
 * + `computeNextRun`) the composer's scheduleCampaign path relies on. This is the
 * single scheduling implementation shared with the Campaign Composer — imports
 * never invent a second scheduler.
 *
 * Returns an error string (or null) so the caller can decide whether to roll the
 * campaign back to a draft when the schedule cannot be saved.
 */
export async function insertCampaignSchedule(
  ...
  try {
    const row = buildScheduleRow(input)
    const { error: deleteError } = await supabase
      .from(CAMPAIGN_SCHEDULES_TABLE)
      .delete()
      .eq('campaign_id', campaignId)
    if (deleteError) {
      return { error: `Failed to clear previous campaign schedule: ${deleteError.message}` }
    }
    const { error } = await supabase
      .from(CAMPAIGN_SCHEDULES_TABLE)
      .insert({ campaign_id: campaignId, ...row })
    if (error) return { error: `Failed to save campaign schedule: ${error.message}` }
    return { error: null }
  } catch (err) {
    return { error: err instanceof Error ? err.message : 'Failed to save campaign schedule' }
  }
}
