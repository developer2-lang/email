...
/**
 * Backfill an automatic follow-up to the original campaign's EXISTING openers.
 * Mirrors backend/followupService.processAutomaticFollowup: when an automatic
 * follow-up is created it is sent immediately to everyone who already opened the
 * original campaign, instead of waiting for a future open event. Runs
 * synchronously (the caller awaits sent/skipped/failed) and never double-sends
 * thanks to the campaign_followup_logs duplicate protection. Used by both the
 * normal Create Follow-up flow and the CSV/Excel Import flow.
 */
async function processAutomaticFollowup(
  config: { campaign_id: string; followup_campaign_id: string }
): Promise<{ sent: number; skipped: number; failed: number }> {
  const { data, error } = await supabase.functions.invoke('send-followup', {
    body: {
      action: 'process_automatic',
  ...
  const body = data as
    ...
    | null
  if (!body || body.success === false) {
    throw new Error(body?.error?.message || 'Failed to process automatic follow-up')
  }
  return body.data || { sent: 0, skipped: 0, failed: 0 }
}
