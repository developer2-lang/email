...
// ─── Follow-up file import: icons + constants (mirrors Campaigns import) ─────
...
const ImportUploadIcon = ({ size = 16 }: { size?: number }) => (
...
const ImportCloseIcon = ({ size = 16 }: { size?: number }) => (
  <svg {...IMPORT_ICON_PROPS} width={size} height={size}>
...
const FOLLOWUP_IMPORT_EXPECTED_HEADERS = [
  'Original Campaign',
  'Follow-up Mode',
  'Follow-up Campaign',
  'Follow-up Name',
...
const FOLLOWUP_CAMPAIGN_CREATE = 'Create a new follow-up campaign'
const FOLLOWUP_CAMPAIGN_REUSE = 'Reuse an existing campaign'

const FOLLOWUP_AUTOMATION_TRUE = ['yes', 'true', '1']
const FOLLOWUP_AUTOMATION_FALSE = ['no', 'false', '0']
...
/** Map a normalized header token to a parsed-row field (case-insensitive / whitespace-tolerant). */
const FOLLOWUP_IMPORT_HEADER_FIELD_MAP: Record<string, keyof FollowupImportFields> = {
  ...
  enablefollowupautomation: 'automation',
}

/** Fields extracted from one Excel/CSV row (before validation). */
interface FollowupImportFields {
...
interface FollowupImportRow extends FollowupImportFields {
  rowNumber: number
  errors: string[]
...
export default function FollowupsTab({
  ...
  // ─── Follow-up file import state ───
  const [isImportModalOpen, setIsImportModalOpen] = useState(false)
  const [importDragging, setImportDragging] = useState(false)
  const [importFileName, setImportFileName] = useState('')
  const [importParsing, setImportParsing] = useState(false)
  const [importRows, setImportRows] = useState<FollowupImportRow[]>([])
  ...
  // ─── Follow-up file import: parsing + validation ──────────────────────────
  const parseImportRows = (rawRows: any[]): FollowupImportRow[] => {
    const campaignByName = new Map<string, any>()
    for (const c of campaigns || []) {
      const name = String(c.name || '').trim().toLowerCase()
      if (name) campaignByName.set(name, c)
    }
    const templateByName = new Map<string, EmailTemplate>()
    for (const t of templates || []) {
      const name = String(t.name || '').trim().toLowerCase()
      if (name) templateByName.set(name, t)
    }

    return rawRows.map((raw, idx) => {
      const fieldValues: Partial<FollowupImportFields> = {}
      Object.keys(raw).forEach((header) => {
        const key = FOLLOWUP_IMPORT_HEADER_FIELD_MAP[normalizeImportHeaderKey(header)]
        if (key && fieldValues[key] === undefined) {
          fieldValues[key] = String(raw[header] ?? '').trim()
        }
      })
      const fields: FollowupImportFields = {
        originalCampaign: fieldValues.originalCampaign || '',
        followupMode: fieldValues.followupMode || '',
        followupCampaign: fieldValues.followupCampaign || '',
        followupName: fieldValues.followupName || '',
        ...
        automation: fieldValues.automation || '',
      }

      const errors: string[] = []
      const rowNumber = idx + 1

      // Original Campaign — must match an existing campaign by name.
      let originalCampaignId = ''
      const origCampaign = fields.originalCampaign
        ? campaignByName.get(fields.originalCampaign.trim().toLowerCase())
        : null
      if (!origCampaign) {
        errors.push(`Original campaign not found: ${fields.originalCampaign || '(empty)'}`)
      } else {
        originalCampaignId = String(origCampaign.id)
      }

      // Follow-up Mode — Manual | Automatic only.
      let modeValue: FollowupMode = 'manual'
      const modeNorm = fields.followupMode.trim().toLowerCase()
      if (modeNorm === 'manual') modeValue = 'manual'
      else if (modeNorm === 'automatic') modeValue = 'automatic'
      else if (fields.followupMode.trim()) {
        errors.push(`Invalid Follow-up Mode: ${fields.followupMode}`)
      } else {
        errors.push('Follow-up Mode is required')
      }

      // Follow-up Campaign — Create new | Reuse existing only.
      const fupCampaignNorm = fields.followupCampaign.trim().toLowerCase()
      let isReuse = false
      if (fupCampaignNorm === FOLLOWUP_CAMPAIGN_CREATE.toLowerCase()) isReuse = false
      else if (fupCampaignNorm === FOLLOWUP_CAMPAIGN_REUSE.toLowerCase()) isReuse = true
      else if (fields.followupCampaign.trim()) {
        errors.push(`Invalid Follow-up Campaign: ${fields.followupCampaign}`)
      } else {
        errors.push('Follow-up Campaign is required')
      }

      if (!fields.followupName.trim()) {
        errors.push('Follow-up Name is required')
      }

      // Campaign Type — must be an existing option.
      let campaignTypeValue = 'Follow Up'
      if (fields.campaignType.trim()) {
        const match = (CAMPAIGN_TYPES as readonly string[]).find(
          (t) => t.toLowerCase() === fields.campaignType.trim().toLowerCase(),
        )
        if (!match) {
          errors.push(`Invalid Campaign Type: ${fields.campaignType}`)
          campaignTypeValue = fields.campaignType.trim()
        } else {
          campaignTypeValue = match
        }
      }
      ...
      let resolvedBody = ''
      if (fields.templateName.trim()) {
        const tpl = templateByName.get(fields.templateName.trim().toLowerCase())
        if (!tpl) {
      ...
      // Resolve the target campaign (reuse) or require the new-campaign fields.
      let reuseCampaignId = ''
      if (isReuse) {
        const existing = fields.followupName.trim()
          ? campaignByName.get(fields.followupName.trim().toLowerCase())
          : null
        if (!existing) {
          errors.push(`Existing follow-up campaign not found: ${fields.followupName || '(empty)'}`)
        } else {
          reuseCampaignId = String(existing.id)
        }
      } else {
        if (!fields.subjectLine.trim()) {
          errors.push('Subject Line is required')
        }
        if (!resolvedBody) {
          errors.push(
            'Email body is required (provide a Template Name to supply the email content)',
          )
        }
      }

      // Enable Follow-up Automation — accepted boolean tokens only.
      let automationValue = true
      if (fields.automation.trim()) {
        const a = fields.automation.trim().toLowerCase()
        if (FOLLOWUP_AUTOMATION_TRUE.includes(a)) automationValue = true
        else if (FOLLOWUP_AUTOMATION_FALSE.includes(a)) automationValue = false
        else {
          errors.push(`Invalid Enable Follow-up Automation: ${fields.automation}`)
        }
      }

      let status: 'valid' | 'invalid' | 'duplicate' = errors.length === 0 ? 'valid' : 'invalid'

      // Pre-flag a reuse target that is already a follow-up campaign (uses the
      // app's existing duplicate/identity rules).
      if (
        status === 'valid' &&
        isReuse &&
        reuseCampaignId &&
        alreadyFollowupCampaignIds.has(reuseCampaignId)
      ) {
        status = 'duplicate'
      }
  ...
  const processImportFile = (file: File) => {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        onToast('Error reading file: ' + (err?.message || 'unknown error'), 'error')
        setImportRows([])
      } finally {
        setImportParsing(false)
      }
  ...
  const confirmFollowupImport = async () => {
    ...
    // Follow-up campaign id of the most recently created Manual row so it can be
    // auto-selected after the import (mirrors normal Create Follow-up).
    let lastManualFollowupId: string | null = null

    for (const row of validRows) {
      const payload: any = {
        original_campaign_id: row.originalCampaignId,
        followup_mode: row.modeValue,
        is_active: row.automationValue,
      }
      if (row.isReuse) {
        payload.followup_campaign_id = row.reuseCampaignId
      } else {
        payload.campaign_name = row.followupName.trim()
        payload.subject_line = row.subjectLine.trim()
        payload.from_name = row.fromName.trim() || DEFAULT_FROM_NAME
        payload.html_content = row.resolvedBody
        payload.campaign_type = row.campaignTypeValue
        payload.template_name = row.resolvedTemplate?.name || null
      }

      try {
        const result = await createFollowupConfig(payload)
        imported++

        const config = result.config
        if (row.modeValue === 'automatic') {
          // Active Automatic follow-ups are backfilled to the original campaign's
          // EXISTING openers immediately — same as the normal Create Follow-up
          // button. Inactive rows are left for the open-tracker / scheduler. A
          // backfill failure must not roll back the (already created) config.
          if (row.automationValue && config && config.campaign_id && config.followup_campaign_id) {
            try {
              const res = await processAutomaticFollowup({
                campaign_id: String(config.campaign_id),
                followup_campaign_id: String(config.followup_campaign_id),
              })
              automaticSends += res.sent
            } catch (backErr: any) {
              failedReasons.push(
                `Row ${row.rowNumber} (automatic backfill): ${backErr?.message || 'failed'}`,
              )
            }
          }
        } else {
          // Manual: do NOT auto-send. Remember the new config so it is selected
          // and its opened recipients shown in the Manual Send section.
          manualReady++
          if (config && config.followup_campaign_id) {
            lastManualFollowupId = String(config.followup_campaign_id)
          }
        }
      } catch (err: any) {
        ...
        if (
          lower.includes('already configured') ||
          lower.includes('cannot be its own') ||
          lower.includes('already')
        ) {
          skipped++
        } else {
          failed++
          failedReasons.push(`Row ${row.rowNumber}: ${msg}`)
        }
      }
    }

    setImportSubmitting(false)
    // Rows already flagged as duplicate during preview are also counted as skipped.
    skipped += importRows.filter((r) => r.status === 'duplicate').length

    await refreshAll()

    // Post-create initialisation, identical to the normal Create Follow-up flow:
    // for a Manual import the new config is selected and its opened recipients are
    // loaded into the Manual Send section; for Automatic the Active row refreshes.
    if (lastManualFollowupId) {
      const fresh = (await fetchFollowupConfigs()).find(
        (c) =>
          String(c.id) === lastManualFollowupId ||
          String(c.followup_campaign_id) === lastManualFollowupId,
      )
      if (fresh) {
        setTab('active')
        handleSelectConfig(fresh)
      }
    }
    ...
    if (imported === 0 && failed > 0) {
      onToast(`Follow-up import failed: ${failedReasons[0]}`, 'error')
    } else {
      const parts: string[] = [`Imported: ${imported}`]
      if (automaticSends > 0) parts.push(`Automatic sends: ${automaticSends}`)
      if (manualReady > 0) parts.push(`Manual follow-ups ready: ${manualReady}`)
      if (skipped > 0) parts.push(`Skipped: ${skipped}`)
      if (failed > 0) parts.push(`Failed: ${failed}`)
      onToast(parts.join(' · '), 'success')
      if (failed > 0 && failedReasons.length > 0) {
        onToast(`Import errors: ${failedReasons.join('; ')}`, 'error')
      }
    }
  }

  const downloadFollowupTemplate = () => {
    const XLSX = (window as any).XLSX
    if (!XLSX) {
      onToast('Excel library not available', 'error')
      return
    }
    const headers = FOLLOWUP_IMPORT_EXPECTED_HEADERS
    const sampleRows = [
      [
        'Immediate Partnership Outreach',
        'Automatic',
    ...
    XLSX.writeFile(wb, 'followup-import-template.xlsx')
  }
    ...
    <div className="page active">
      <div className="fu-header">
        ...
        <div className="fu-header-actions">
          ...
          >
            <ImportUploadIcon size={15} /> Import File
          </button>
      ...
      {/* ─── MODAL: IMPORT FOLLOW-UPS FROM FILE ─── */}
      {isImportModalOpen && (
        <div className="modal-overlay">
          <div className="modal modal-wide">
            <div className="modal-header">
              <div>
                <div className="modal-title">Import Follow-ups from File</div>
                <div className="ct-sub" style={{ marginTop: 3 }}>
                  Upload an Excel or CSV file to bulk create follow-ups
                </div>
              </div>
              <button
              ...
              >
                <div style={{ flex: 1, minWidth: 260 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--text4)', marginTop: 8 }}>
                    Use these column names in your Excel file. One row represents one follow-up.
                  </div>
                </div>
                <button
                  className="btn btn-secondary"
                  onClick={downloadFollowupTemplate}
                  style={{ whiteSpace: 'nowrap' }}
                >
                  <ImportUploadIcon size={15} /> Download Excel Template
                </button>
              </div>
                      ...
                      <>
                        ...
                        >
                          ...
                          <span style={{ fontWeight: 700, color: 'var(--green)' }}>
                            {importRows.length} rows parsed · {valid} valid · {invalid} invalid ·{' '}
                            {duplicate} duplicate
                          </span>
                        </div>
                        <div
                        ...
                        >
                          <table
                            style={{
                              fontSize: 11,
                              width: '100%',
                              minWidth: `${FOLLOWUP_IMPORT_EXPECTED_HEADERS.length * 130}px`,
                            }}
                          >
                            <thead>
                              <tr>
                                {FOLLOWUP_IMPORT_EXPECTED_HEADERS.map((key) => (
                                  <th
                                    key={key}
                                    style={{
                                      padding: '8px 10px',
                            ...
                            <tbody>
                              {importRows.slice(0, 5).map((row) => (
                                <tr key={row.rowNumber}>
                                  {FOLLOWUP_IMPORT_EXPECTED_HEADERS.map((key) => {
                                    const fieldKey =
                                      FOLLOWUP_IMPORT_HEADER_FIELD_MAP[
                                        normalizeImportHeaderKey(key)
                                      ] as keyof FollowupImportFields
                        ...
                        <div style={{ marginTop: 14 }}>
                          {valid > 0 && (
                            ...
                            >
                              {valid} follow-up{valid === 1 ? '' : 's'} ready to import
                            </div>
                          )}
                          {invalid > 0 && (
                            <div
                            ...
                            >
                              {invalid} follow-up{invalid === 1 ? '' : 's'} has errors
                            </div>
                          )}
                          {duplicate > 0 && (
                            <div
                            ...
                            >
                              {duplicate} follow-up{duplicate === 1 ? '' : 's'} will be skipped
                              (duplicate)
                            </div>
                          )}
                              ...
                              {importRows
                                ...
                                .map((row) => (
                                  ...
                                  >
                                    ...
                                    >
                                      Row {row.rowNumber}
                                      {row.status === 'duplicate' ? ' — Duplicate follow-up' : ''}
                                    </div>
                                    {row.errors.map((err, i) => (
                                      <div
                                        key={i}
                                        style={{ color: 'var(--red)', paddingLeft: 6 }}
                                      >
                                        • {err}
                                      </div>
                                    ))}
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={
                  ...
                  importParsing ||
                  importSubmitting ||
                  importRows.filter((r) => r.status === 'valid').length === 0
                }
                onClick={() => void confirmFollowupImport()}
              >
                {importSubmitting ? 'Importing…' : 'Confirm & Import Follow-ups'}
              </button>
            </div>
          </div>
        </div>
