...
import { toEmailSafeHtml, wrapHtmlDocument } from '../utils/emailRender';
...
export default function TemplateEditorTab({ onToast, onNavigate }: TemplateEditorTabProps) {
  ...
  // ─── Import File (load an HTML file directly into the existing editor) ───
  ...
  /**
   * Load raw HTML (from an imported file) into the EXISTING TemplateVisualEditor
   * — reusing the exact same single-centered-container pipeline used by
   * `loadTemplateIntoEditor` so the imported template renders through the same
   * email-safe renderer as everything else. No database/storage write happens;
   * the imported template behaves like a freshly created (unsaved) template, so
   * the user chooses when to Save Template / Save As.
   */
  const loadImportedHtml = useCallback(
    (rawHtml: string, name: string) => {
      try {
        const html = toEmailSafeHtml(rawHtml);
        loadedHtmlRef.current = html;
        setHtmlContent(html);
        setSelectedTemplate(null);
        setMode('visual');
        setEditorKey((k) => k + 1);
        setDirty(false);
        onToast(`'${name}' imported — edit it, then Save Template to keep it.`, 'success');
      } catch (err) {
  ...
  /**
   * Parse + validate a selected file with the SAME logic the Upload HTML path
   * uses, but WITHOUT saving it. Surfaces the exact spec'd error messages:
   *  - wrong extension → "Invalid file type. Please upload an HTML template…"
   *  - unparseable HTML → "Unable to read this HTML template. Please check…"
   */
  const handleImportFileSelected = async (file: File | null | undefined) => {
    if (!file) return;
    setImportReading(true);
    setImportError(null);
    setImportValid(false);
    try {
      const { html, name } = await readHtmlTemplateFile(file);
      setImportFile(file);
      setImportHtml(html);
      setImportName(name);
      setImportValid(true);
      ...
      setImportHtml('');
      setImportName('');
      setImportError(err instanceof Error ? err.message : 'Unable to read this HTML template. Please check the file and try again.');
    } finally {
      setImportReading(false);
    }
  };
  ...
  const confirmImportFile = () => {
    if (!importValid || !importHtml) return;
    const html = importHtml;
    const name = importName;
    setImportOpen(false);
    loadImportedHtml(html, name);
  };
  ...
  return (
    <div className={fullscreen ? 'page active te-fs-page' : 'page active'}>
      ...
      >
        ...
        <button
          type="button"
          style={{ ...toolButton, color: '#0F766E', borderColor: '#99F6E4', background: '#F0FDFA' }}
          onClick={openImportModal}
          title="Import an HTML template file into the editor"
        >
          ⤓ Import File
        </button>
      ...
      {/* ─── Import File (load HTML into the existing editor) ─── */}
      {importOpen && (
        <div className="modal-overlay">
          <div className="modal" style={{ width: '640px', maxWidth: '94vw' }}>
            <div className="modal-header">
              <div className="modal-title">Import Template from File</div>
              <button className="btn-icon" onClick={() => setImportOpen(false)}>
                ✕
              </button>
            </div>
            <div className="modal-body">
              <div style={{ fontSize: '12.5px', color: '#64748B', marginBottom: '16px' }}>
                Upload an HTML template file to import it into the Template Editor
              </div>
              ...
              <input
                ...
                hidden
                accept=".html,.htm,text/html"
                onChange={(e) => void handleImportFileSelected(e.target.files?.[0])}
              />

              {!importFile && !importReading && !importError && (
                <div
                  ...
                  onDrop={(e) => {
                    e.preventDefault();
                    setImportDragActive(false);
                    void handleImportFileSelected(e.dataTransfer.files?.[0]);
                  }}
                  style={{
                    display: 'flex',
                    flexDirection: 'column',
                    ...
                    gap: '10px',
                    padding: '40px 20px',
                    border: `2px dashed ${importDragActive ? '#0F766E' : '#CBD5E1'}`,
                    borderRadius: '14px',
                    background: importDragActive ? '#F0FDFA' : '#F8FAFC',
                    cursor: 'pointer',
                    textAlign: 'center',
                    transition: 'border-color 0.15s ease, background 0.15s ease',
                  }}
                >
                  <div style={{ fontSize: '30px', color: '#0F766E' }}>⤓</div>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#0F172A' }}>
                    Drag &amp; drop HTML file here
                  </div>
                  <div style={{ fontSize: '12.5px', color: '#64748B' }}>or</div>
                  <div
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
              ...
              {importReading && (
                ...
                >
                  <span className="seqb-spin" style={{ fontSize: '20px', color: '#0F766E' }}>
                    ⟳
                  </span>
                  Reading file…
                </div>
              ...
              {importFile && importValid && (
                <>
                  ...
                  >
                    <div style={{ fontSize: '12.5px', color: '#134E4A', fontWeight: 600 }}>
                  ...
                  <iframe
                    ...
                    sandbox=""
                    srcDoc={wrapHtmlDocument(toEmailSafeHtml(importHtml))}
                    style={{
                      width: '100%',
                      height: '320px',
                      border: '1px solid #E2E8F0',
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={!importValid || !importHtml || importReading}
              >
                Import Template
              </button>
            </div>
