...
// ─── Sequence file import: icons + constants (mirrors the Follow-ups importer) ─
const SEQ_IMPORT_ICON_PROPS = {
  viewBox: '0 0 24 24',
  fill: 'none',
  stroke: 'currentColor',
  strokeWidth: 2,
...
const SeqImportUploadIcon = ({ size = 16 }: { size?: number }) => (
  <svg {...SEQ_IMPORT_ICON_PROPS} width={size} height={size}>
...
const SeqImportCloseIcon = ({ size = 16 }: { size?: number }) => (
  <svg {...SEQ_IMPORT_ICON_PROPS} width={size} height={size}>
...
const SEQUENCE_IMPORT_EXPECTED_HEADERS = [
  'Step Number',
  'Parent Step',
  'Parent Branch',
  'Recipient Type',
...
function normalizeSeqHeaderKey(header: string): string {
...
const SEQUENCE_IMPORT_HEADER_FIELD_MAP: Record<string, keyof SequenceImportFields> = {
...
interface SequenceImportFields {
  stepNumber: string;
  parentStep: string;
  parentBranch: string;
...
type SeqImportBranch = 'STARTING' | 'OPENED' | 'NOT_OPENED';

/** A fully parsed + validated import row with its computed status. */
interface SequenceImportRow extends SequenceImportFields {
  rowNumber: number;
  /** Numeric portion of Step Number (siblings like 2 / 2A share this). */
  baseStep: number | null;
  /** Parent Step number the user typed (resolved to a node id at import time). */
  parentBaseStep: number | null;
  /** The branch this node extends (from the Parent Branch column). */
  branch: SeqImportBranch | null;
  recipientType: RecipientType;
  /** Wait hours as a number (0 when empty). */
  waitHoursVal: number;
  /** Resolved templates.id for the Template Name (null when none/empty). */
...
export default function SequencesTab({ onPersistSequences, onToast }: SequencesTabProps) {
  ...
  // ─── SEQUENCE FILE IMPORT STATE ───
  const [isImportOpen, setIsImportOpen] = useState(false);
  const [importDragging, setImportDragging] = useState(false);
  const [importFileName, setImportFileName] = useState('');
  const [importParsing, setImportParsing] = useState(false);
  const [importRows, setImportRows] = useState<SequenceImportRow[]>([]);
  const [importSubmitting, setImportSubmitting] = useState(false);
  const [importSeqName, setImportSeqName] = useState('');
  ...
  const resetImportState = () => {
    ...
    setImportSeqName('');
    setImportAudience(audienceOptions[0]?.label || '');
  };
  ...
  const parseImportRows = (rawRows: any[]): SequenceImportRow[] => {
    const templateByName = new Map<string, string>();
    for (const t of templates || []) {
      const name = String(t.name || '').trim().toLowerCase();
      if (name) templateByName.set(name, t.id);
    }

    const parsed = rawRows.map((raw, idx) => {
      const fieldValues: Partial<SequenceImportFields> = {};
      Object.keys(raw).forEach((header) => {
        const key = SEQUENCE_IMPORT_HEADER_FIELD_MAP[normalizeSeqHeaderKey(header)];
        if (key && fieldValues[key] === undefined) {
          fieldValues[key] = String(raw[header] ?? '').trim();
        }
      });
      const fields: SequenceImportFields = {
        stepNumber: fieldValues.stepNumber || '',
        parentStep: fieldValues.parentStep || '',
        parentBranch: fieldValues.parentBranch || '',
        recipientType: fieldValues.recipientType || '',
        subject: fieldValues.subject || '',
        fromName: fieldValues.fromName || '',
        templateName: fieldValues.templateName || '',
        body: fieldValues.body || '',
        waitHours: fieldValues.waitHours || '',
      };
      ...
      // Step Number
      const baseStepStr = fields.stepNumber.replace(/[^0-9]/g, '');
      const baseStep = baseStepStr ? Number(baseStepStr) : null;
      if (!baseStep || !Number.isFinite(baseStep) || baseStep <= 0) {
        errors.push('Step Number is required and must be a positive number');
      }

      // Parent Branch
      const branchNorm = fields.parentBranch.trim().toUpperCase().replace(/[_\s]/g, '');
      let branch: SeqImportBranch | null = null;
      if (branchNorm === 'STARTING') branch = 'STARTING';
      else if (branchNorm === 'OPENED') branch = 'OPENED';
      else if (branchNorm === 'NOTOPENED' || branchNorm === 'NOT_OPENED') branch = 'NOT_OPENED';
      else if (fields.parentBranch.trim()) {
        errors.push(`Invalid Parent Branch: ${fields.parentBranch}`);
      } else {
        errors.push('Parent Branch is required');
      }

      // Parent Step
      const parentBaseStepStr = fields.parentStep.replace(/[^0-9]/g, '');
      const parentBaseStep = parentBaseStepStr ? Number(parentBaseStepStr) : null;
      if (branch === 'STARTING') {
        if (fields.parentStep.trim()) {
          errors.push('Starting step must not have a Parent Step');
        }
      } else if (branch) {
        if (!parentBaseStep) {
          errors.push('Parent Step is required for a branched step');
        }
      }
      ...
      // Recipient Type
      const recNorm = fields.recipientType.trim().toLowerCase().replace(/[_\s]/g, '');
      let recipientType: RecipientType = 'all';
      if (recNorm === 'opened') recipientType = 'opened';
      else if (recNorm === 'notopened' || recNorm === 'not_opened') recipientType = 'not_opened';
      else if (recNorm === 'all' || recNorm === '') recipientType = 'all';
      else errors.push(`Invalid Recipient Type: ${fields.recipientType}`);

      // Template Name (optional)
      let resolvedTemplateId: string | null = null;
      if (fields.templateName.trim()) {
        const tplId = templateByName.get(fields.templateName.trim().toLowerCase());
        if (!tplId) errors.push(`Template not found: ${fields.templateName}`);
        else resolvedTemplateId = tplId;
      }

      // Wait Hours
      let waitHoursVal = 0;
      if (fields.waitHours.trim()) {
        const w = Number(fields.waitHours.trim());
        if (!Number.isFinite(w) || w < 0 || !Number.isInteger(w)) {
          errors.push(`Invalid Wait Hours: ${fields.waitHours}`);
        } else {
          waitHoursVal = w;
        }
      }
      ...
      return {
        ...
        resolvedTemplateId,
        errors,
        status: (errors.length === 0 ? 'valid' : 'invalid') as 'valid' | 'invalid',
      };
    });

    // Cross-row parent resolution: every non-root node must reference an existing
    // parent node (keyed by Parent Step number + branch). The root (STARTING)
    // parent node is always its STARTING node regardless of the child's branch.
    const nodeKeys = new Set<string>();
    for (const r of parsed) {
      if (r.baseStep && r.branch) nodeKeys.add(`${r.baseStep}#${r.branch}`);
    }
    for (const r of parsed) {
      if (r.status !== 'valid') continue;
      if (r.branch === 'STARTING') continue;
      const parentIsRoot = parsed.some(
        (x) => x.baseStep === r.parentBaseStep && x.branch === 'STARTING',
      );
      const parentKey = parentIsRoot
        ? `${r.parentBaseStep}#STARTING`
        : `${r.parentBaseStep}#${r.branch}`;
      if (!nodeKeys.has(parentKey)) {
        r.errors.push(`Parent Step ${r.parentBaseStep} (${r.branch}) not found`);
        r.status = 'invalid';
      }
    }
  ...
  const processImportFile = (file: File) => {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const readRows = (rows: any[]) => {
          setImportRows(parseImportRows(rows));
          setImportParsing(false);
        };
        if (ext === 'csv') {
          const wb = XLSX.read(String(e.target.result ?? ''), { type: 'string' });
          readRows(XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval: '' }));
        } else {
  ...
  /** Build the sequence-step payload for one imported row, resolving the parent
   *  node's UUID to the actual database step id. Mirrors the manual builder's
   *  stepPayload / stepNotOpenedPayload so the imported records are identical to
   *  manually-created step nodes. */
  const buildStepPayload = (
    row: SequenceImportRow,
    parentId: string | null,
  ): SequenceStepInput => {
    if (row.branch === 'NOT_OPENED') {
      return {
        step_number: row.baseStep == null ? undefined : row.baseStep,
        normal_subject: '',
        normal_body: '',
        normal_template_id: null,
        increment_subject: row.subject.trim() || null,
        increment_body: row.body.trim() || null,
        increment_template_id: row.resolvedTemplateId,
        from_name: row.fromName.trim() || null,
        wait_hours: row.waitHoursVal,
        recipient_type: row.recipientType,
        parent_step_id: parentId,
        parent_branch: 'NOT_OPENED',
      };
    }
    // STARTING (root) or OPENED branch node.
    return {
      step_number: row.baseStep == null ? undefined : row.baseStep,
      normal_subject: row.subject.trim(),
      normal_body: row.body.trim(),
      normal_template_id: row.resolvedTemplateId,
      increment_subject: null,
      increment_body: null,
      increment_template_id: null,
      from_name: row.fromName.trim() || null,
      wait_hours: row.branch === 'STARTING' ? row.waitHoursVal : 0,
      recipient_type: row.recipientType,
      parent_step_id: parentId,
      parent_branch: row.branch,
    };
  };

  const confirmImport = async () => {
    if (importSubmitting) return;
    if (!importSeqName.trim()) {
      onToast('Sequence name is required', 'error');
      return;
    }
    const validRows = importRows.filter((r) => r.status === 'valid');
    if (validRows.length === 0) {
      onToast('No valid rows to import', 'error');
      return;
    }
    setImportSubmitting(true);
    try {
      const audience = importAudience.trim() || audienceOptions[0]?.label || 'Imported';
      const created = await createSequence({
        name: importSeqName.trim(),
        audience_segment: audience,
        trigger_type: 'manual',
        recipient_type: 'all',
        send_mode: 'automatic',
      });
      const sequenceId = created.id;
      const ordered = [...validRows].sort((a, b) => (a.baseStep || 0) - (b.baseStep || 0));
      const nodeKeyToId: Record<string, string> = {};
      for (const row of ordered) {
        const nodeKey = `${row.baseStep}#${row.branch}`;
        let parentId: string | null = null;
        if (row.branch !== 'STARTING') {
          const parentIsRoot = ordered.some(
            (x) => x.baseStep === row.parentBaseStep && x.branch === 'STARTING',
          );
          const parentKey = parentIsRoot
            ? `${row.parentBaseStep}#STARTING`
            : `${row.parentBaseStep}#${row.branch}`;
          parentId = nodeKeyToId[parentKey] || null;
        }
        const payload = buildStepPayload(row, parentId);
        const step = await createStep(sequenceId, payload);
        if (row.baseStep != null && row.branch) {
          nodeKeyToId[nodeKey] = step.id;
        }
      }
      onToast(
        `Sequence "${created.name}" imported with ${validRows.length} step${
      ...
      onToast(
        err instanceof Error ? err.message : 'Failed to import sequence',
        'error',
      );
    } finally {
      setImportSubmitting(false);
  ...
  const downloadSeqTemplate = () => {
    const XLSX = (window as any).XLSX;
    if (!XLSX) {
      onToast('Excel library not available', 'error');
      return;
    }
    const headers = SEQUENCE_IMPORT_EXPECTED_HEADERS;
    const sample = [
      ['1', '', 'Starting', 'all', 'Welcome to the sequence', 'Your Name', '', 'Hi {{first_name}}, thanks for joining!', '0'],
      ['2', '1', 'Opened', 'opened', 'Glad you opened', 'Your Name', '', 'Great that you opened our email.', '0'],
      ['2A', '1', 'Not Opened', 'not_opened', 'Did you see this?', 'Your Name', '', 'We noticed you have not opened — reminder.', '24'],
      ['3', '2', 'Opened', 'opened', 'Next step', 'Your Name', '', 'Here is the next resource.', '0'],
      ['3A', '2', 'Not Opened', 'not_opened', 'Final reminder', 'Your Name', '', 'Final nudge if still not opened.', '48'],
    ];
    const ws = XLSX.utils.aoa_to_sheet([headers, ...sample]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sequence Steps');
    XLSX.writeFile(wb, 'sequence-import-template.xlsx');
  };
  ...
  return (
    <div>
      ...
      <div className="seq-header">
        ...
        <button
          className="btn btn-secondary btn-new-seq"
          onClick={() => openImport()}
          style={{ marginLeft: '10px' }}
        >
          <SeqImportUploadIcon size={15} /> Import File
        </button>
      </div>
      ...
      {isImportOpen && (
        <div className="modal-overlay">
          <div className="modal modal-wide">
            <div className="modal-header">
              <div>
                <div className="modal-title">Import Sequence from File</div>
                <div className="ct-sub" style={{ marginTop: 3 }}>
                  Upload an Excel or CSV file to bulk create a sequence and its steps
                </div>
              </div>
              <button
            ...
            <div className="modal-body">
              ...
              <div className="grid-2" style={{ marginBottom: '16px' }}>
                <div className="form-group" style={{ marginBottom: '0' }}>
                  <label>
                    Sequence Name <span style={{ color: 'var(--red)' }}>*</span>
                  </label>
                  <input
                    type="text"
                    value={importSeqName}
                    onChange={(e) => setImportSeqName(e.target.value)}
                    placeholder="e.g. Post-launch drip imported from Excel"
                  />
                </div>
                <div className="form-group" style={{ marginBottom: '0' }}>
                  <label>Target Audience</label>
                  ...
                  >
                    <option value="">Use default (first audience)</option>
                    {audienceOptions.map((a) => (
                      <option key={a.id} value={a.label}>
                        {a.label}
                      </option>
              ...
              >
                <div style={{ flex: 1, minWidth: 260 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {SEQUENCE_IMPORT_EXPECTED_HEADERS.map((col) => (
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--text4)', marginTop: 8 }}>
                    Use these column names in your Excel file. One row represents one branch step.
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  >
                    <SeqImportUploadIcon size={15} /> Choose File
                  </button>
                  <button
                    className="btn btn-secondary"
                    onClick={downloadSeqTemplate}
                    style={{ whiteSpace: 'nowrap' }}
                  >
                    <SeqImportUploadIcon size={15} /> Download Excel Template
                  </button>
                </div>
              </div>
                      ...
                      <>
                        ...
                        >
                          <table
                            style={{
                              fontSize: 11,
                              width: '100%',
                              minWidth: `${SEQUENCE_IMPORT_EXPECTED_HEADERS.length * 110}px`,
                            }}
                          >
                            <thead>
                              <tr>
                                {SEQUENCE_IMPORT_EXPECTED_HEADERS.map((key) => (
                                  <th
                                    key={key}
                                    style={{
                                      padding: '8px 10px',
                            ...
                            <tbody>
                              {importRows.slice(0, 5).map((row) => (
                                <tr key={row.rowNumber}>
                                  {SEQUENCE_IMPORT_EXPECTED_HEADERS.map((key) => {
                                    const fieldKey =
                                      SEQUENCE_IMPORT_HEADER_FIELD_MAP[
                                        normalizeSeqHeaderKey(key)
                                      ] as keyof SequenceImportFields;
                                  ...
                                  <td style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>
                                    <span
                                      className={`tag ${
                                        row.status === 'valid' ? 'tag-client' : 'tag-draft'
                                      }`}
                                    >
                        ...
                        <div style={{ marginTop: 14 }}>
                          {valid > 0 && (
                            ...
                            >
                              {valid} step{valid === 1 ? '' : 's'} ready to import
                            </div>
                          )}
                          {invalid > 0 && (
                            <div
                            ...
                            >
                              {invalid} step{invalid === 1 ? '' : 's'} has errors
                            </div>
                          )}
                          {invalid > 0 && (
                            <div
                              ...
                              {importRows
                                ...
                                .map((row) => (
                                  ...
                                  >
                                    ...
                                    >
                                      Row {row.rowNumber} — Step {row.stepNumber || '(empty)'}
                                    </div>
