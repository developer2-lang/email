...
import { contactMatchesSegment } from '../utils/contactSegment';
import type {
  AudienceOption,
  BranchStepInput,
  ManualSendResult,
...
// Audience resolution is no longer an exact-match map. Segment names are labels;
// the actual contacts are derived from the Contacts table by category prefix
// (see src/utils/contactSegment). Matching is case-insensitive so values like
// "Existing Client (Vatsal/ Shubham)" / "New Client - Inbound" resolve correctly.
...
async function resolveContactsForAudience(audienceSegment: unknown): Promise<any[]> {
  ...
  const contacts = data || [];
  // "All Contacts" (or empty) → every contact.
  if (!segment || segment.toLowerCase() === 'all contacts') return contacts;
  return contacts.filter((c: any) => contactMatchesSegment(c, segment));
}
