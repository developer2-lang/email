...
export async function createFollowupConfig(payload = {}) {
  ...
  // Automatic mode delivers IMMEDIATELY on creation — including openers who
  // already opened the original campaign before this follow-up existed
  // (Trigger 1). We AWAIT the backfill so the response reflects the REAL send
  // outcome and is never reported as successful unless sendFollowupEmail
  // actually returned success. The existing send pipeline is reused; no second
  // SMTP sender is introduced.
  let automaticResult = null;
  if (config && mode === 'automatic' && isActive) {
    automaticResult = await processAutomaticFollowup(config);
  }
  ...
  return {
    ...
    automatic: automaticResult
      ? {
          existing_openers: automaticResult.existing_openers,
          sent: automaticResult.sent,
          skipped: automaticResult.skipped,
          failed: automaticResult.failed,
          // Metrics for the Follow-ups page (recalculated, never guessed):
          openers: automaticResult.existing_openers,
          eligible: Math.max(0, automaticResult.existing_openers - automaticResult.sent),
          delivered: automaticResult.sent,
        }
      : null,
  };
}
...
async function createAllCampaignsFollowup(payload) {
  ...
  // per-campaign configurations are preserved untouched. For automatic mode we
  // remember the linked originals so we can backfill their openers afterward.
  ...
  if (isActive) {
    for (const campaign of eligible) {
      ...
      linkedOriginals.push(String(campaign.id));
    }
  }

  // Automatic mode delivers to existing openers immediately (backfill). We
  // AWAIT every linked original's backfill so the response reflects the REAL
  // send outcome and is never reported as successful unless sendFollowupEmail
  // actually returned success.
  let automaticResult = null;
  if (mode === 'automatic' && isActive) {
    let existingOpeners = 0;
    let sent = 0;
    let skipped = 0;
    let failed = 0;
    for (const originalId of linkedOriginals) {
      const r = await processAutomaticFollowup({
        campaign_id: originalId,
        followup_campaign_id: followupCampaignId,
      });
      existingOpeners += r.existing_openers;
      sent += r.sent;
      skipped += r.skipped;
      failed += r.failed;
    }
    automaticResult = {
      existing_openers: existingOpeners,
      sent,
      skipped,
      failed,
      openers: existingOpeners,
      eligible: Math.max(0, existingOpeners - sent),
      delivered: sent,
    };
  }
...
export async function updateFollowupConfig(configId, payload = {}) {
  ...
  // Re-activated as automatic → backfill any openers not yet successfully sent.
  if (data && mode === 'automatic' && isActive) {
    processAutomaticFollowup(data).catch((err) =>
      console.error(`[Followup] Automatic backfill failed for ${data.campaign_id}: ${err.message}`)
    );
  }
  return data || null;
}
...
async function updateAllFollowup(followupCampaignId, payload) {
  ...
  if (Object.keys(updates).length > 0 && (rows || []).length > 0) {
    ...
    // Re-activated as automatic → backfill each linked original's openers.
    const nowAutomatic = mode === 'automatic' && (payload.is_active === undefined || payload.is_active === true);
    if (nowAutomatic) {
      for (const row of rows || []) {
        processAutomaticFollowup(row).catch((err) =>
          console.error(`[Followup] Automatic all-campaigns backfill failed for ${row.campaign_id}: ${err.message}`)
        );
      }
    }
  }
 ...
 * Pending Follow-ups tab reflects the outcome.
 *
 * Returns a result object so callers can distinguish a GENUINE successful send
 * from a skip or a failure — the caller must NOT count a follow-up as "sent"
 * unless `sendFollowupEmail` actually returned success (status === 'sent').
 *
 * @returns {Promise<{status:'sent'|'skipped'|'failed', alreadySent:boolean, log:object|null, error?:string}>}
 */
async function sendAutomaticFollowup(campaignId, contactId, email, followupCampaignId, openedAt) {
  ...
  if (log && ['sent', 'already_sent'].includes(log.status)) {
    ...
    return { status: 'skipped', alreadySent: true, log };
  }
  // A pending/failed row is NOT treated as successfully sent — it is retried.
  if (!log) {
    log = await insertPendingLog({ campaignId, contactId, email, followupCampaignId, openedAt });
    if (!log) return { status: 'skipped', alreadySent: true, log: null }; // concurrent insert won the race
  }
  ...
  try {
    ...
    return { status: 'sent', alreadySent: false, log };
  } catch (error) {
    ...
    return { status: 'failed', alreadySent: false, log, error: error.message };
  }
}

/**
 * The automatic follow-up processor.
 *
 * The live trigger (handleOpenFollowup) only fires on a *new* open event recorded
 * AFTER the follow-up is configured. Openers who already opened the ORIGINAL
 * campaign before the follow-up existed would therefore never be delivered to —
 * their campaign_followup_logs rows sit at `pending` forever and the follow-up
 * is effectively "skipped". This processor closes that gap: it delivers the
 * follow-up to every opener of the original campaign who has NOT yet successfully
 * received it.
 *
 * Duplicate protection is identical to the live trigger: a recipient is skipped
 * ONLY when their campaign_followup_logs row for the (original, follow-up) pair
 * is already `sent` or `already_sent` (the UNIQUE (campaign_id, contact_id,
 * followup_campaign_id) constraint guarantees at most one successful send). A row
 * left in `pending` or `failed` — e.g. a prior attempt that errored — is retried,
 * because a failed send must NEVER count as successfully sent and must NEVER
 * permanently block the recipient.
 *
 * The actual delivery reuses the existing send path (sendAutomaticFollowup →
 * sendFollowupEmail); this function is orchestration only — no second send
 * implementation.
 *
 * @param {{campaign_id:string, followup_campaign_id:string}} config
 * @returns {Promise<{sent:number, skipped:number, failed:number}>}
 */
/**
 * The automatic follow-up processor for EXISTING openers (Trigger 1).
 *
 * Queries the ORIGINAL campaign's email_logs for every recipient whose
 * original-campaign email has opened=true, then pushes each one through the
 * EXISTING automatic send pipeline (sendAutomaticFollowup → sendFollowupEmail).
 * Eligibility is STRICTLY "original campaign opened=true" — unopened recipients,
 * the full audience, a segment, or every contact are never emailed.
 *
 * A recipient is skipped ONLY when their campaign_followup_logs row for the
 * (original, follow-up) pair is already `sent` or `already_sent`. A row left in
 * `pending` or `failed` is retried, because a failed send must NEVER count as
 * successfully sent and must NEVER permanently block the recipient.
 *
 * The actual delivery reuses the existing send path; this function is
 * orchestration only — no second send implementation.
 *
 * @param {{campaign_id:string, followup_campaign_id:string}} config
 * @returns {Promise<{existing_openers:number, sent:number, skipped:number, failed:number}>}
 */
export async function processAutomaticFollowup(config) {
  if (!config || !config.campaign_id || !config.followup_campaign_id) {
    return { existing_openers: 0, sent: 0, skipped: 0, failed: 0 };
  }
  const originalId = String(config.campaign_id);
  ...
  const { data: openedLogs, error } = await supabase
    ...
    .eq('campaign_id', originalId)
    .eq('opened', true);
  if (error) {
    console.error(`[Followup] processAutomaticFollowup could not read openers for ${originalId}: ${error.message}`);
    return { existing_openers: 0, sent: 0, skipped: 0, failed: 0 };
  }

  const existingOpeners = (openedLogs || []).length;
  console.log('[FOLLOWUP AUTOMATIC CREATE]', JSON.stringify({
    followup_id: followupCampaignId,
    original_campaign_id: originalId,
    existing_openers_count: existingOpeners,
  }));
  ...
  for (const log of openedLogs || []) {
    const contactId = String(log.contact_id);
    const email = log.email || '';
    if (!email) {
      skipped += 1;
      continue;
    }
    const openedAt = log.opened_at || new Date().toISOString();

    const existing = await getExistingLog(originalId, contactId, followupCampaignId);
    const alreadySent = Boolean(existing && ['sent', 'already_sent'].includes(existing.status));

    const result = await sendAutomaticFollowup(originalId, contactId, email, followupCampaignId, openedAt);
    let sendResult;
    ...
    console.log('[FOLLOWUP AUTOMATIC EXISTING OPENER]', JSON.stringify({
      contact_id: contactId,
      email,
      opened: true,
  ...
  return { existing_openers: existingOpeners, sent, skipped, failed };
}

/**
 * Process every active automatic follow-up configuration: deliver to any opener
 * of the original campaign that has not yet successfully received the follow-up.
 *
...
export async function runAutomaticFollowups() {
  const { data: configs, error } = await supabase
    ...
    .select('*')
    .eq('is_active', true)
    .eq('followup_mode', 'automatic');
  if (error) {
    if (error.code === '42P01') return { processed: 0, sent: 0, skipped: 0, failed: 0 };
    throw toError(error, 'Failed to list automatic follow-up configurations');
  }
  ...
  for (const cfg of configs || []) {
    const result = await processAutomaticFollowup(cfg);
    processed += 1;
    sent += result.sent;
...
export async function handleOpenFollowup(campaignId, contactId, email) {
  ...
  if (config.followup_mode === 'automatic') {
    const result = await sendAutomaticFollowup(campaignId, contactId, email, followupCampaignId, openedAt);
    console.log('[FOLLOWUP AUTOMATIC LIVE OPEN]', JSON.stringify({
      contact_id: contactId,
      email,
